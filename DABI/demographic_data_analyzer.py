...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = round(((higher_education) & (df['salary']=='>50K')).sum()/higher_education.sum()*100,1)
    lower_education_rich = round(((~higher_education) & (df['salary']=='>50K')).sum()/((~higher_education).sum())*100,1)
    ...
    highest_earning_country = ((country_rich/country_all)*100).idxmax()
    highest_earning_country_percentage = round(((country_rich/country_all)*100).max(),1)
